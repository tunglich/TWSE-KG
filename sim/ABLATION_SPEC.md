...
> |------|--------|---------------------|
> | `ablation_design.py`, `calibrated_sim.py`, `shuffle_mechanism.py` | **Design simulation only** — uses a latent-Gaussian model calibrated to paper F1 values to estimate statistical power before running the real experiment | **NO** |
...
> | `exp/precheck_out.csv` | **Real output** — correlation check on actual KG node pairs | **YES** |
>
